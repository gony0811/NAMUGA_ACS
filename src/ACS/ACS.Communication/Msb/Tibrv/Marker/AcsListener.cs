...
{
    public class AcsListener : GenericWorkflowTibrvCmListener
    {
        public override void onMsg(object tibrvListener, MessageReceivedEventArgs tibrvMsg)
        {
            try
            {
                ...
                if (obj is XmlDocument)
                {
                    ...
                    string messageName = document.DocumentElement["Command"].InnerText;
                    if(messageName ==null)
                    {
                        logger.Error("document format is not valid, Command Text Empty");
                        return;
                    }

                    //XmlElement header = document.DocumentElement["Command"];
                    //if (header == null)
                    //{
                    //    logger.Error("document format is not valid, HEADER should exist");
                    //    return;
                    //}
                    //string messageName = header["Command"] != null ? header["MESSAGENAME"].InnerText : "";
