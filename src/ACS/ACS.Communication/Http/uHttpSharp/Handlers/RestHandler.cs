...
namespace ACS.Communication.Http.uHttpSharp.Handlers
{
    public class RestHandler<T> : IHttpRequestHandler
    {
        ...
        static RestHandler()
        {
            ...
            RestCallHandlers.Add(RestCall.Create(HttpMethods.Post, false), async (c, r) => (object) (await c.Post(r)));     //SSM
            RestCallHandlers.Add(RestCall.Create(HttpMethods.Post, true), async (c, r) => (object)(await c.Post(r)));
            ...
            //RestCallHandlers.Add(RestCall.Create(HttpMethods.Post, "1D1F"), async (c, r) => (object)(await c.Post(r)));
            //RestCallHandlers.Add(RestCall.Create(HttpMethods.Post, "1D2F"), async (c, r) => (object)(await c.Post(r)));
            //RestCallHandlers.Add(RestCall.Create(HttpMethods.Post, "2D1F"), async (c, r) => (object)(await c.Post(r)));
            //RestCallHandlers.Add(RestCall.Create(HttpMethods.Post, "2D2F"), async (c, r) => (object)(await c.Post(r)));
        }
        ...
        public async Task Handle(IHttpContext httpContext, Func<Task> next)
        {
            ...
            var call = new RestCall(httpRequest.Method, httpRequest.RequestParameters.Length > 1);      // 'acs_mod/{ID}.. ID => Controller Matching Check -> RestHandler()

            Func<IRestController<T>, IHttpRequest, Task<object>> handler;
            if (RestCallHandlers.TryGetValue(call, out handler))        // Check Controller
            {
                var value = await handler(_controller, httpRequest).ConfigureAwait(false);      // Controller Execute -> RestHandler() -> c.Post(r)
