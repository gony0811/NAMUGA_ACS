...
namespace ACS.Control
{
    public class ControlServerManagerImplement : AbstractManager, IControlServerManager
    {
        ...
        public override void Init()
        {
            ...
            StartConfigurationFilePath = "config/@{site}/startup/startup.xml";
        ...
        public string GetDestinationName(string applicationName)
        {
            ...
            Application application = this.ApplicationManager.GetApplication(applicationName);
        ...
        public bool Start(string applicationName, string applicationType)
        {
            ...
            document.Load(this.StartConfigurationFilePath);

            if (document == null)
            {
                //logger.error("failed to start, please check configuration file");
                return false;
            }

            bool existId = false;
            XmlNodeList ids = document.SelectNodes("//id");
            XmlElement idElement = null;
            foreach (XmlNode id in ids)
            {
                if (id.InnerText.Equals(applicationName))
                {
                    idElement = (XmlElement)id;
                    existId = true;
                }
            }
            ...
            if (!existId)
            {
                //logger.error("failed to start, please check id");
                return false;
            }

            String memory = idElement.GetAttribute("memory");
            String jmx = idElement.GetAttribute("jmx");
