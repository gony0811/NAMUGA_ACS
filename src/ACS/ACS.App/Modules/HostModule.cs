...
namespace ACS.App.Modules
{
    ...
    public class HostModule : Module
    {
        protected override void Load(ContainerBuilder builder)
        {
            ...
            // === Host 전용: TCP 게이트웨이 + 메시지 서비스 + 브릿지 서비스 ===
            ...
            builder.RegisterType<ACS.App.Host.HostMessageService>()
                .As<IHostMessageService>()
                .SingleInstance();

            // 브릿지 BackgroundService (TCP ↔ Workflow)
            builder.RegisterType<ACS.App.Host.HostBridgeService>()
                .As<IHostedService>()
                .AsSelf()
                .SingleInstance();
        }
