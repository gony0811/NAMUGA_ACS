...
{
    class RAIL_VEHICLEMAINBOARDVERSION : BaseBizJob
    {
        ...
        {
            XmlDocument rail_vehiclemainboardversion = (XmlDocument)args[0];
            ...
            vehicleMsg = InterfaceService.CreateVehicleMessageFromES(rail_vehiclemainboardversion);
            if(ResourceService.CheckVehicle(vehicleMsg))
            {
                ...
                ResourceService.UpdateVehicleMainboardVersion(vehicleMsg);
                return 0;
            }
            else
            {
