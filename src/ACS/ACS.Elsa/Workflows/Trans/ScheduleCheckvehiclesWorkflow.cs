...
namespace ACS.Elsa.Workflows.Trans
{
    ...
    ///   3. RecoverStuckVehicles (RUN+STOP vehicle 에 carrierTransfer 재전송)
    /// </summary>
    public class ScheduleCheckvehiclesWorkflow : WorkflowBase
    {
        protected override void Build(IWorkflowBuilder builder)
        {
            ...
            builder.Description = "Vehicle EventTime 검사: 1분 이상 미갱신 Vehicle DISCONNECT 처리 + RUN+STOP vehicle carrierTransfer 재전송";
            ...
            builder.Root = new Sequence
            {
                Activities =
                {
                    ...
                    // Step 3: RUN+STOP 으로 멈춘 vehicle 에 carrierTransfer 재전송
                    new RecoverStuckVehiclesActivity()
                }
            };
        }
    }
