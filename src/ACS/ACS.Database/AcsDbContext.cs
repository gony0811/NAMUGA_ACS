...
// Application domain
using AppModel = ACS.Framework.Application.Model;
// Alarm domain
...
namespace ACS.Database
{
    ...
    public class AcsDbContext : DbContext
    {
        ...
        protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
        {
            ...
            {
                string connectionString = ConfigurationManager.AppSettings["HibernateConnectionString"];
                string dbType = ConfigurationManager.AppSettings[ACS.Framework.Application.Settings.SYSTEM_DATABASE_TYPE];

                if (string.Equals(dbType, "SQLITE", StringComparison.OrdinalIgnoreCase)
                    || string.IsNullOrEmpty(dbType))
                {
                    optionsBuilder.UseSqlite(connectionString ?? "Data Source=acs.db");
                }
                // Oracle, SQL Server 등은 향후 필요 시 NuGet 패키지 추가 후 확장
            }
        }
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            // 필요 시 ACS.Framework에 Secs 모델 클래스를 생성하여 추가.
        }
    }
}
