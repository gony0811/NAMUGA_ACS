...
3. **exchangeCmd 오케스트레이션 전환(S8)**: `docs/ACS-AMR_mqtt_exchangecmd.docx` 협의 #2~#7 확정 후 — TS 가 단일 exchangeCmd 로 여정 위임, EI 단계 보고(STEP_COMPLETE 등) 라우팅→TS 미러링, C3 returnNode 전달, 시뮬레이터 exchangeCmd 시퀀스. **EI 프로토콜 계층(모델/발행/파싱)은 §53 에서 선반영 완료**
>>>>>>> Stashed changes
