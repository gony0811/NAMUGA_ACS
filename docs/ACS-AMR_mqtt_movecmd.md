...
| `portType` | string | - | 포트 유형 (ACS LocationEx.Type 값을 그대로 송신: `EQP` / `BUFFER` / `INPUT` / `OUTPUT` / `CHARGE` / `VBUFFER`). 미지정 시 자재포트로 간주 |
...
`portType` 값에 따라 AMR 측 시퀀스 동작이 달라진다. ACS는 LocationEx.Type 값을 그대로 송신하며, AMR은 아래 카테고리로 분기한다.

- **설비포트 카테고리**: `EQP` — 도착 후 ACS의 ActionCmd 수신까지 최대 120초 대기
- **자재포트 카테고리**: `BUFFER` / `INPUT` / `OUTPUT` / `VBUFFER` — 대기 없이 다음 단계 진행
- **충전소 카테고리**: `CHARGE` — 충전 동작 수행
- **미지정/공백**: 자재포트로 간주

| 단계 | EQP (설비포트) | BUFFER/INPUT/OUTPUT/VBUFFER (자재포트) | CHARGE (충전소) |
|------|----------------|-----------------------------------------|------------------|
| 도착 후 동작 | ActionCmd 수신까지 최대 120초 대기 | 대기 없이 다음 단계 진행 | 충전 시퀀스 진입 |

실제 분기 로직: `AMR/Service/MoveSequenceRunner.cs:577-587` (LocationEx.Type 값 기준으로 동기화 필요)
...
**portType 카테고리 표기 (이하 표에서 사용):**
- `EQP` 카테고리 = `EQP`
- `자재` 카테고리 = `BUFFER` / `INPUT` / `OUTPUT` / `VBUFFER`

### Step 8 — PICK

| jobType | portType 카테고리 | PICK 대상 | DI 번호 | 결정 변수 |
|---------|-------------------|-----------|---------|-----------|
| LOAD    | (무관)            | AMR 슬롯에서 PICK | `0 + amrSlotOffset` (DI0~DI3) | `amrSlot` |
| UNLOAD  | EQP               | 설비포트에서 PICK | `10 + portSlotOffset` (LEFT=DI10, RIGHT=DI11) | `port` |
| UNLOAD  | 자재              | 자재포트에서 PICK | `14 + portSlotOffset` (LEFT=DI14, RIGHT=DI15) | `port` |

### Step 9 — PLACE

| jobType | portType 카테고리 | PLACE 대상 | DI 번호 | 결정 변수 |
|---------|-------------------|------------|---------|-----------|
| LOAD    | EQP               | 설비포트에 PLACE | `8 + portSlotOffset` (LEFT=DI8, RIGHT=DI9) | `port` |
| LOAD    | 자재              | 자재포트에 PLACE | `12 + portSlotOffset` (LEFT=DI12, RIGHT=DI13) | `port` |
| UNLOAD  | (무관)            | AMR 슬롯에 PLACE | `4 + amrSlotOffset` (DI4~DI7) | `amrSlot` |

### 고정 DI

| 동작 | portType 카테고리 | DI 번호 | 단계 |
|------|-------------------|---------|------|
| QR 읽기 위치 이동 | EQP   | DI16 | Step 6 |
| QR 읽기 위치 이동 | 자재  | DI17 | Step 6 |
